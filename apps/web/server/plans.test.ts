...
describe('public Sell Plan decoding', () => {
  it('reads only the public Plan projection from validated account bytes', () => {
    ...
    view.setBigUint64(154, 2_500_000n, true);
  ...
  it('decodes canceled Plans without treating returned stock as sold', () => {
    const data = new Uint8Array(238);
    data.set([161, 231, 251, 119, 2, 12, 162, 2]);
    new DataView(data.buffer).setUint16(8, 1, true);
    data[212] = 2;
    new DataView(data.buffer).setBigUint64(154, 2_500_000n, true);
    expect(decodePublicSellPlan(data, 'plan')).toMatchObject({
      status: 'canceled',
      remainingRawInventory: '0',
      soldRawInventory: '2500000',
    });
  });
});
